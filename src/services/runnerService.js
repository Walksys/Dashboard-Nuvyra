...
class RunnerService {
  ...
  syncMinecraftProperties(serverDir, targetPort) {
    ...
    try {
      ...
      if (!fs.existsSync(eulaPath)) {
        fs.writeFileSync(eulaPath, '# EULA accepted by Nuvyra\neula=true\n', 'utf8');
      } else {
      ...
      console.warn('[Nuvyra] Warning updating eula.txt:', err.message);
    }
    ...
    try {
      ...
      if (!fs.existsSync(propsPath)) {
        const defaultProps = [
          '# Minecraft server properties generated by Nuvyra',
          'motd=Powered by Nuvyra',
          `server-port=${port}`,
          `query.port=${port}`,
          'server-ip=',
          'online-mode=true',
      ...
      console.warn('[Nuvyra] Warning updating server.properties:', err.message);
    }
  }
  ...
  async startServer(serverId) {
    ...
    this.appendLog(sId, `\r\n\x1b[36m[Nuvyra]\x1b[0m Starting server ${server.name}...\r\n`);
    ...
    if (server.server_type === 'minecraft') {
      ...
      if (!fs.existsSync(jarPath)) {
        this.appendLog(sId, `\x1b[33m[Nuvyra]\x1b[0m ${jarFile} missing! Automatically downloading ${server.jar_type || 'paper'} (${mcVersion}, build ${buildNumber})...\r\n`);
        try {
          ...
          this.appendLog(sId, `\x1b[32m[Nuvyra]\x1b[0m Successfully installed ${jarFile}!\r\n`);
        } catch (jarErr) {
          this.appendLog(sId, `\x1b[31m[Nuvyra Error]\x1b[0m Could not download ${jarFile}: ${jarErr.message}\r\n`);
        }
      }
    }
    ...
    if (isMinecraft) {
      ...
      this.appendLog(sId, `\x1b[36m[Nuvyra]\x1b[0m Synchronized Minecraft port: server-port=${targetPort}, query.port=${targetPort}\r\n`);
    }
    ...
    if (dockerService.isAvailable) {
      try {
        this.appendLog(sId, `\x1b[32m[Nuvyra]\x1b[0m Containerizing with ${server.docker_image}...\r\n`);
        ...
        this.appendLog(sId, `\x1b[33m[Nuvyra Warning]\x1b[0m Docker spawn issue (${dockErr.message}). Switching to native runner.\r\n`);
      }
    }

    // Fallback native process runner
    try {
      this.appendLog(sId, `\x1b[32m[Nuvyra]\x1b[0m Executing command: ${startupCmd}\r\n`);
      ...
      child.on('close', (code) => {
        this.appendLog(sId, `\r\n\x1b[31m[Nuvyra]\x1b[0m Server process stopped with exit code ${code}.\r\n`);
        record.process = null;
        if (!record.isRestarting) {
          record.status = 'offline';
          query.run('UPDATE servers SET status = ? WHERE id = ?', ['offline', sId]).catch(() => {});
      ...
      child.on('error', (err) => {
        this.appendLog(sId, `\r\n\x1b[31m[Nuvyra Error]\x1b[0m ${err.message}\r\n`);
      });
  ...
  async stopServer(serverId) {
    ...
    this.appendLog(sId, `\r\n\x1b[33m[Nuvyra]\x1b[0m Stopping server...\r\n`);
  ...
  async killServer(serverId) {
    ...
    await query.run('UPDATE servers SET status = ? WHERE id = ?', ['offline', sId]);
    this.broadcast(sId, { type: 'status', status: 'offline' });
    this.appendLog(sId, `\r\n\x1b[31m[Nuvyra]\x1b[0m Server was forcefully terminated.\r\n`);
    return { success: true };
  }
  ...
  async restartServer(serverId) {
    ...
    this.appendLog(sId, `\r\n\x1b[33m[Nuvyra]\x1b[0m Server is restarting...\r\n`);
    ...
    try {
      ...
      for (const c of containers) {
        for (const name of (c.Names || [])) {
          const match = name.match(/nuvyra-server-(\d+)-/);
          if (match) {
  ...
  async sendCommand(serverId, command) {
    ...
    if ((!record || record.status !== 'running') && dockerService.isAvailable && dockerService.docker) {
      try {
        const containers = await dockerService.docker.listContainers();
        const match = containers.find(c => (c.Names || []).some(n => n.includes(`nuvyra-server-${sId}-`)));
        if (match) {
